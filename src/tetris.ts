...
import { updateTetris, moveLeft, moveRight, moveDown, rotate, hardDrop, lockAndSpawn } from './game/tetris/update'
...
function handleScore(score: number, highScore: number, linesCleared: number): void {
  playEat()
  ...
  if (linesCleared >= 4) {
    playMilestone(); triggerShake(gameWrapper)
...
function startIdleDemo(): void {
  ...
  const reset = () => { Object.assign(demo, createTetrisState()); demo.running = true; demo.autopilot = true }
...
function doHardDrop(): void {
  ...
  if (points > 0 || linesCleared > 0) handleScore(state.score, state.highScore, linesCleared)
...
document.addEventListener('keydown', (e) => {
  if (state.running && !state.paused && !state.autopilot) {
    switch (e.code) {
      case 'ArrowLeft':  case 'KeyA': moveLeft(state);   break
      case 'ArrowRight': case 'KeyD': moveRight(state);  break
      case 'ArrowDown':  case 'KeyS': moveDown(state); state.dropTimer = 0; break
      case 'ArrowUp':    case 'KeyW': rotate(state,  1); break
      case 'KeyZ':                   rotate(state, -1); break
      case 'Space': doHardDrop(); break
    }
...
document.addEventListener('touchend', e => {
  if (touchX === null || touchY === null || !state.running || state.paused || state.autopilot) return
  const dx = e.changedTouches[0].clientX - touchX
  const dy = e.changedTouches[0].clientY - touchY
  touchX = touchY = null
  if (Math.abs(dx) < 20 && Math.abs(dy) < 20) return
  if (Math.abs(dy) > Math.abs(dx)) { if (dy > 0) doHardDrop(); else rotate(state, 1) }
  else                              { if (dx > 0) moveRight(state); else moveLeft(state) }
}, { passive: true })

// D-pad
document.getElementById('d-left')?. addEventListener('click', () => { if (state.running && !state.paused && !state.autopilot) moveLeft(state) })
document.getElementById('d-right')?.addEventListener('click', () => { if (state.running && !state.paused && !state.autopilot) moveRight(state) })
document.getElementById('d-rotate')?.addEventListener('click', () => { if (state.running && !state.paused && !state.autopilot) rotate(state, 1) })
document.getElementById('d-down')?. addEventListener('click', () => { if (state.running && !state.paused && !state.autopilot) doHardDrop() })
...
lbBtn?.addEventListener('click', async () => {
  lbBtn.disabled = true; lbBtn.textContent = '...'
  try {
    renderLeaderboard(overlay, await fetchTop(10, 'leaderboard_tetris'))
  ...
  } finally { lbBtn.disabled = false; lbBtn.textContent = '🏆 Visualizza Top 10' }
})
...
audioBtn?.addEventListener('click', () => {
  const en = !isAudioEnabled(); setAudioEnabled(en)
...
crtBtn?.addEventListener('click', () => {
  ...
  crtBtn.classList.toggle('active', on); crtBtn.setAttribute('aria-pressed', String(on))
})
