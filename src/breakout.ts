...
function startIdleDemo(): void {
  ...
  demo.autopilot = true;
  const reset = () => {
    Object.assign(demo, createBreakoutState());
    demo.running = true;
    demo.autopilot = true;
  };
...
setInterval(() => {
  if (state.running) updateLives(state.lives);
}, 100);
...
document.addEventListener(
  "touchmove",
  (e) => {
    if (touchX === null || !state.running || state.paused || state.autopilot)
      return;
    ...
    state.paddleX = Math.max(
      0,
      Math.min(W - state.paddleW, state.paddleX + dx * scale),
    );
  },
  { passive: true },
);
...
// D-pad
document
  .getElementById("d-left")
  ?.addEventListener("pointerdown", () => held.add("ArrowLeft"));
document
  .getElementById("d-right")
  ?.addEventListener("pointerdown", () => held.add("ArrowRight"));
